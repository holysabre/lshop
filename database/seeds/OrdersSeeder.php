    ...
    public function run()
    {
        ...
        foreach ($orders as $order){
            ...
            $items = factory(OrderItem::class, random_int(1, 3))->create([
                'order_id' => $order->id,
                'rating'   => $order->reviewed ? random_int(1, 5) : null, // 随机1-5分
                'review'   => $order->reviewed ? $faker->sentence : null,
                'reviewed' => $order->reviewed ? $faker->dateTimeBetween($order->paid_at) : null,// 评价时间不能早于支付时间
            ]);
            ...
            if($order->coupon_code_id){
                $total = $order->coupon_code->getAdjustPrice($total);
            }
