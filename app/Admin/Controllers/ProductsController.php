...
class ProductsController extends AdminController
{
    ...
    protected function form()
    {
        ...
        // 直接添加一对多的关联模型
        $form->hasMany('product_skus', 'SKU 列表', function(Form\NestedForm $form){
            $form->text('title', 'SKU 名称')->rules('required');
            $form->text('description', 'SKU 描述')->rules('required');
            $form->text('price', 'SKU 单价')->rules('required|numeric|min:0.01');
            $form->text('stock', 'SKU 剩余库存')->rules('required|integer|min:0');
        });

        // 定义事件回调，当模型即将保存时会触发这个回调
        $form->saving(function (Form $form) {
            $form->model()->price = collect($form->input('product_skus'))->where(Form::REMOVE_FLAG_NAME, 0)->min('price') ?: 0;
        });
